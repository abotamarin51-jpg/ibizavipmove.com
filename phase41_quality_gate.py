...
for p in HTML:
    ...
    if canonical in canonical_to_file:
        errors.append(f'duplicate canonical output: {canonical} -> {canonical_to_file[canonical]} and {p}')
    canonical_to_file[canonical]=p
    indexable.append((p,text,canonical))

# Self-heal sitemap parity: retain metadata for valid URLs, remove stale URLs, add missing canonicals.
...
if sitemap.exists():
    try:
        ...
        warnings.append(f'could not parse previous sitemap; rebuilding: {exc}')

root=ET.Element(f'{{{NS}}}urlset')
for canonical in sorted(canonical_to_file):
    u=ET.SubElement(root,f'{{{NS}}}url')
    ET.SubElement(u,f'{{{NS}}}loc').text=canonical
    meta=metadata.get(canonical,{})
    if meta.get('lastmod'): ET.SubElement(u,f'{{{NS}}}lastmod').text=meta['lastmod']
    if meta.get('changefreq'): ET.SubElement(u,f'{{{NS}}}changefreq').text=meta['changefreq']
    if meta.get('priority'): ET.SubElement(u,f'{{{NS}}}priority').text=meta['priority']
ET.ElementTree(root).write(sitemap,encoding='utf-8',xml_declaration=True)

# Internal paths. Historical low-value broken links are warnings; critical core navigation is checked below.
...
def path_exists(path):
    ...
    clean=path.lstrip('/')
    return (ROOT/clean).exists() or (ROOT/clean/'index.html').exists()
...
for p,text,canonical in indexable:
    for href in re.findall(r'<a\b[^>]*\bhref="([^"]+)"',text,re.I):
        ...
        if target and not path_exists(target):
            warnings.append(f'legacy/noncritical internal target from {p.relative_to(ROOT)}: {href}')

    # Local visual assets are critical because missing images damage the luxury experience.
    ...
    # Phase 35 accessibility landmarks must persist.
...
# Explicitly verify the new five-language clusters and reciprocal hreflang we control.
clusters=[
...
for cluster in clusters:
    urls=[BASE+p for p in cluster]
    missing=[u for u in urls if u not in canonical_to_file]
    if missing:
        errors.append(f'incomplete five-language cluster: {missing}')
        continue
    expected=set(urls)
    for url in urls:
        ...
        if actual!=expected:
            errors.append(f'hreflang cluster mismatch on {url}: expected {sorted(expected)}, found {sorted(actual)}')

# Core conversion/navigation targets must always exist.
...
# Contact forms must retain the IDs used by the WhatsApp workflow.
...
if len(warnings)>30: print(f'WARN: {len(warnings)-30} additional noncritical legacy-link warning(s) suppressed')
if errors:
    print('\n'.join('FAIL: '+e for e in errors))
    raise SystemExit(f'Phase 41 quality gate found {len(errors)} critical issue(s)')

print(f'PASS: Phase 41 quality gate — {len(indexable)} indexable pages; sitemap auto-healed; critical assets, accessibility, multilingual clusters, navigation and forms verified; {len(warnings)} noncritical warning(s)')

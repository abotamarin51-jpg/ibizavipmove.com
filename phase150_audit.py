"""Read-only gate for Phase 150 localized dining/nightlife discovery."""
...
def require(ok: bool, message: str) -> None:
    if not ok:
        raise SystemExit('Phase 150 audit: ' + message)
...
def run(root: Path = ROOT) -> None:
    ...
    for lang, (label, href) in PAGES.items():
        ...
        require(html.count(linked_phrase(label, href)) == 1, f'exact linked phrase: {lang}')
        tags = Tags(html).tags
        actual = [a.get('href') for t, a in tags if t == 'a' and a.get('data-ivm150') == 'service']
        require(actual == [href], f'exact one dining/nightlife link: {lang}')
        slug = href.strip('/')
        target_tags = Tags((root / slug / 'index.html').read_text(encoding='utf-8')).tags
        canonical = BASE + href
        require(canonical in urls, f'sitemap membership: {slug}')
        require([a.get('href') for t, a in target_tags if t == 'link' and a.get('rel') == 'canonical'] == [canonical], f'self-canonical: {slug}')
        require(sum(t == 'h1' for t, a in target_tags) == 1, f'one H1: {slug}')
        require(not any(t == 'meta' and a.get('name', '').lower() in {'robots', 'googlebot'} and 'noindex' in a.get('content', '').lower() for t, a in target_tags), f'indexability: {slug}')
        require({a.get('hreflang') for t, a in target_tags if t == 'link' and a.get('rel') == 'alternate'} == LANGS, f'hreflang: {slug}')
    ...
    require(all(depths.get(url, 999) <= 2 for url in targets), 'dining/nightlife targets within two HTML links of English Home')

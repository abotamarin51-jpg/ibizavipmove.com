...
if not cookies_path.is_file() or not privacy_path.is_file():
    raise SystemExit('Expected privacy and cookie policy pages are missing')
...
# Build guards: every generated HTML page must carry the consent layer, and the
# policy pages must accurately describe the production setup.
for path in html_files:
    text = path.read_text(encoding='utf-8')
    if SCRIPT_TAG not in text:

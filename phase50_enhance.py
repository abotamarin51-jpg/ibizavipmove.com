...
NAV={
'es':{
 'services':('/es/servicios/','Servicios'),'concierge':('/es/concierge-privado-ibiza/','Concierge'),'chauffeur':('/es/chauffeur-privado-ibiza/','Chófer'),'office':('/es/private-office/','Private Office'),'contact':('/es/contacto/','Contacto'),'cta':'WhatsApp 24/7','menu':'Menú'},
'fr':{
 'services':('/fr/services/','Services'),'concierge':('/fr/conciergerie-privee-ibiza/','Conciergerie'),'chauffeur':('/fr/chauffeur-prive-ibiza/','Chauffeur'),'office':('/fr/private-office/','Private Office'),'contact':('/fr/contact/','Contact'),'cta':'WhatsApp 24/7','menu':'Menu'},
'de':{
 'services':('/de/services/','Services'),'concierge':('/de/privater-concierge-ibiza/','Concierge'),'chauffeur':('/de/privater-chauffeur-ibiza/','Chauffeur'),'office':('/de/private-office/','Private Office'),'contact':('/de/kontakt/','Kontakt'),'cta':'WhatsApp 24/7','menu':'Menü'},
'ar':{
 'services':('/ar/services/','الخدمات'),'concierge':('/ar/private-concierge-ibiza/','الكونسيرج'),'chauffeur':('/ar/private-chauffeur-ibiza/','السائق الخاص'),'office':('/ar/private-office/','Private Office'),'contact':('/ar/contact/','التواصل'),'cta':'واتساب 24/7','menu':'القائمة'}
}
...
for lang,d in NAV.items():
    ...
    assert (ROOT/d['office'][0].strip('/')/'index.html').exists(),(lang,'localized private office missing')
    for file in ROOT.rglob('*.html'):
        html=file.read_text(encoding='utf-8')
        lm=re.search(r'<html\b[^>]*\blang=["\']([a-zA-Z-]+)["\']',html,re.I)
        ...
        assert d['office'][0] in n,(lang,file,'office')

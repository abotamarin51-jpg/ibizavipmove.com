...
## 13 September 2026 — Phase 135: localized Services contact routing

Evidence: the existing `/fr/services/`, `/de/services/` and `/ar/services/` hubs each exposed two primary conversion CTAs to the English `/contact/` page even though their localized contact routes already existed. Navigation elsewhere on those hubs already used the localized destinations, so the primary conversion path was inconsistent with the language journey.

Published change: PR #59 merged as `4641c63df77e00ec7220ebc9503cd17a10835c51`. Only those six CTA destinations are normalized to `/fr/contact/`, `/de/kontakt/` and `/ar/contact/`. No visible copy, schema, sitemap, tracking, WhatsApp destination, service scope, price, legal policy, URL inventory or Google Business Profile data changed.

Validation: PR CI `34755777513` passed all existing and new gates. Production deployment `34755813017` / job `103719876889` completed successfully, including GitHub Pages and the existing IndexNow step. Exact production artifact `10317925991` has SHA256 `d1566520f3ff9bc70316267f96bca998e79f71149c9b1b77cb0796d960330161`. Artifact inspection confirms zero English `/contact/` links on the three hubs, four localized contact links per hub, one self-canonical and H1 each, existing localized contact targets, and exactly 156 unique sitemap URLs. A clean build also re-encoded the existing `villa.jpg` bytes without changing its dimensions or source code; this encoder variance is not part of the Phase 135 functional change. Fresh live on-page checks returned HTTP 200 and indexable status for all three hubs with self-canonicals, six hreflang entries, mobile viewport and no reported issues.

Impact: technical conversion/localization coherence only. Search Console data is settled only through 10 September, so no ranking, traffic, lead or revenue uplift is attributable to this release. Next priority is to audit any remaining cross-language conversion links independently while preserving the Phase 134 plan to reassess the two recently crawled commercial indexation exclusions on or after 16 September unless a new concrete failure appears earlier.

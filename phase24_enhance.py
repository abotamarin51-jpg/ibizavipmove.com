...
'msg':{'hello':'Hola Ibiza VIP Move,','request':'Me gustaría solicitar asistencia privada en Ibiza.','name':'Nombre','phone':'Teléfono','arrival':'Llegada','departure':'Salida','guests':'Huéspedes','service':'Servicio','brief':'Brief','tbc':'Por confirmar','fallback':'Me gustaría comentar los detalles de forma privada.','close':'Por favor, indíquenme disponibilidad y próximos pasos. Gracias.'}},
'fr': {
...
'msg':{'hello':'Bonjour Ibiza VIP Move,','request':'Je souhaite demander une assistance privée à Ibiza.','name':'Nom','phone':'Téléphone','arrival':'Arrivée','departure':'Départ','guests':'Invités','service':'Service','brief':'Brief','tbc':'À confirmer','fallback':'Je souhaite discuter des détails en privé.','close':'Merci de m’indiquer les disponibilités et les prochaines étapes.'}},
'de': {
...
'msg':{'hello':'Hallo Ibiza VIP Move,','request':'Ich möchte private Unterstützung auf Ibiza anfragen.','name':'Name','phone':'Telefon','arrival':'Anreise','departure':'Abreise','guests':'Gäste','service':'Service','brief':'Briefing','tbc':'Noch offen','fallback':'Ich möchte die Details gerne privat besprechen.','close':'Bitte teilen Sie mir Verfügbarkeit und die nächsten Schritte mit. Vielen Dank.'}},
'ar': {
...
'msg':{'hello':'مرحباً Ibiza VIP Move،','request':'أرغب في طلب مساعدة خاصة في إيبيزا.','name':'الاسم','phone':'الهاتف','arrival':'الوصول','departure':'المغادرة','guests':'الضيوف','service':'الخدمة','brief':'التفاصيل','tbc':'سيتم التأكيد','fallback':'أرغب في مناقشة التفاصيل بشكل خاص.','close':'يرجى إخباري بالتوفر والخطوات التالية. شكراً.'}}
}
...
def localized_handler(d):
    cfg=json.dumps(d['msg'],ensure_ascii=False)
    return f'''<script>(function(){{const f=document.getElementById('localizedConciergeForm');if(!f)return;const cfg={cfg};const g=id=>document.getElementById(id)?.value||'';const a=document.getElementById('fArrival');const dep=document.getElementById('fDeparture');const now=new Date();const today=new Date(now.getTime()-now.getTimezoneOffset()*60000).toISOString().slice(0,10);if(a)a.min=today;if(dep)dep.min=today;if(a&&dep){{const sync=()=>{{dep.min=a.value||today;if(dep.value&&a.value&&dep.value<a.value)dep.value='';}};a.addEventListener('change',sync);sync();}}f.addEventListener('submit',function(e){{e.preventDefault();if(!f.reportValidity())return;const service=g('fService')||cfg.tbc;if(typeof window.ivmTrack==='function')window.ivmTrack('private_brief_submit',{{service:service,cta_placement:'localized_contact_form'}});const lines=[cfg.hello,'',cfg.request,'',cfg.name+': '+g('fName'),cfg.phone+': '+g('fPhone'),cfg.arrival+': '+(g('fArrival')||cfg.tbc),cfg.departure+': '+(g('fDeparture')||cfg.tbc),cfg.guests+': '+(g('fGuests')||cfg.tbc),cfg.service+': '+service,'',cfg.brief+':',g('fBrief')||cfg.fallback,'',cfg.close];window.location.href='{WA}?text='+encodeURIComponent(lines.join('\n'));}});}})();</script>'''

def footer(cta,d):
    return f'''<footer><div class="footer-grid"><div><div class="footer-brand">IBIZA VIP MOVE</div><p>Private concierge · Ibiza</p></div><div><h4>Contact</h4><a href="tel:+34600703303">{PHONE}</a><a href="{WA}">{cta}</a></div><div><h4>Explore</h4><a href="/private-office/">Private Office</a><a href="/ibiza-intelligence/">The Ibiza Black Book</a><a href="/international-clients/">International Clients</a></div></div><div class="footer-bottom"><span>© 2026 Ibiza VIP Move</span><span>Discretion · Precision · Ibiza</span></div></footer><div class="mobile-bar"><a href="tel:+34600703303">Call</a><a href="{WA}">WhatsApp</a></div><script src="/assets/premium.js?v=24"></script>{localized_handler(d)}'''
...
# Route Contact links / header concierge CTA on language pages to localized private brief.
for lang,d in DATA.items():
    lang_root=ROOT/lang
    if not lang_root.exists():
        continue
    for p in lang_root.rglob('*.html'):
        if p == ROOT/d['path'].lstrip('/')/'index.html':
            continue
        txt=p.read_text(encoding='utf-8')
        txt=txt.replace('href="/contact/"',f'href="{d["path"]}"')
        txt=re.sub(r'(<a\s+class="nav-cta"\s+href=")[^"]+("[^>]*>)',rf'\1{d["path"]}\2',txt,count=1,flags=re.I)
        p.write_text(txt,encoding='utf-8')

# Add new URLs to sitemap after Phase 15 has already applied lastmod to prior URLs.
...
# Keep llms.txt supplementary summary current.
llms=ROOT/'llms.txt'; lt=llms.read_text(encoding='utf-8'); marker='## Localized private brief pages\n'
if marker not in lt:
    lt += '\n'+marker
...
# Release validation.
for lang,d in DATA.items():
    target=ROOT/d['path'].lstrip('/')/'index.html'; txt=target.read_text(encoding='utf-8')
    assert txt.count('<h1')==1, f'{lang}: expected one H1'
    assert f'<link rel="canonical" href="{BASE}{d["path"]}">' in txt, f'{lang}: canonical missing'
    assert '<form id="localizedConciergeForm">' in txt, f'{lang}: localized form missing'
    assert 'private_brief_submit' in txt, f'{lang}: conversion event missing'
    assert 'hreflang="en"' in txt and 'hreflang="x-default"' in txt, f'{lang}: hreflang missing'
    if lang=='ar': assert 'dir="rtl"' in txt, 'Arabic brief must remain RTL'
...
print('PASS: Phase 24 localized private brief pages + localized WhatsApp submit + hreflang + sitemap')
